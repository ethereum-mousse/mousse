        ...
        for shard in 0..SHARD_NUM as Shard {
            ...
            let low_fee_bid = Bid {
                ...
                commitment: DataCommitment::dummy_from_bytes(
                    &format!("Bid with a low fee: Slot {}, Shard {}", slot, shard).into_bytes(),
                ),
                fee: 1,
            };
            let high_fee_bid = Bid {
                ...
                slot,
                commitment: DataCommitment::dummy_from_bytes(
                    &format!("Bid with a high fee: Slot {}, Shard {}", slot, shard).into_bytes(),
                ),
                fee: 21000 * 100,
            };
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("Bench process_slots_happy(1000)", |b| {
        b.iter(process_1000_slots_happy)
    });
    group.bench_function("Bench process_slots_happy(1000) with bids", |b| {
        b.iter(process_1000_slots_happy_with_bids)
    });
    group.bench_function("Bench process_slots_random(1000)", |b| {
        b.iter(process_1000_slots_random)
    });
    group.bench_function("Bench process_slots_random(1000) with bids", |b| {
        b.iter(process_1000_slots_random_with_bids)
    });
